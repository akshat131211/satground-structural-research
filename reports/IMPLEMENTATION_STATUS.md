...
- **One complete reviewed mask pair has now been exported and evaluated.** Following the earlier assisted corrections, the user painted 413 additional building pixels and explicitly requested that those additions be scored. A separate accepted packet extends validity by exactly 411 pixels; the other two were already valid. The user's submitted building mask, target image, and original saved version remain unchanged. The accepted scoring region covers 27,095 of 65,536 pixels (41.3%). Raw responses, exact masks, and verification records remain local.
- Re-evaluated the same saved GJ and GD predictions with this one reviewed target and the other 23 original pseudo-label targets. All other views' metrics and all image-quality metrics were verified unchanged. On the reviewed view, GJ/GD building IoU is 0.806923/0.827901 and boundary error is 0.082974/0.082898. Changed scores versus the original evaluation reflect changed labels and scoring regions, not changed models. This single previously inspected view does not establish an accuracy gain or pass full data QA. See the [separate diagnostic](reviewed-label-probe/report.md).
- The editor now identifies the review view, provides general image-region presets, and reports how many newly painted building pixels the validity mask actually scores. This feedback does not alter either mask. View 2 has a saved user building correction removing 1,231 truck-region pixels. Its validity mask still scored 701 of those pixels. An isolated exclusion proposal was prepared, but the user chose to edit validity themselves. The editor is in validity mode; no view-2 reviewed index or evaluation has been created.
...
- Latest targeted checks: **11 editor/annotation tests passed**, plus JavaScript drawing and independent scoring-coverage checks. The earlier complete suite passed 50 tests with two optional CUDA tests skipped; the renderer is unchanged by this reporting/editor update.
