"""Evaluate existing checkpoints serially, without training or replacing prior results."""
import argparse
import os
...
def run(output, resume=False):
    ...
    files = [manifest, manual, 'data/style/learning100-train-mean.json',
             'docs/CHECKPOINT_DIAGNOSIS.md', 'scripts/run_checkpoint_diagnosis.py']
    ...
    for name in ('A1', 'A3'):
        files.append(f'configs/exploratory500/{name}.yaml')
        files.extend(f'runs/exploratory500-seed17/{name}-train/step-{step:06d}.pt'
    ...
    try:
        for step in (100, 200, 300, 400):
            for name in ('A1', 'A3'):
                ...
                generate = [*base, 'generate', '--config', f'configs/exploratory500/{name}.yaml',
                            '--manifest', manifest, '--data-root', 'data/vigor',
                            '--checkpoint', f'runs/exploratory500-seed17/{name}-train/step-{step:06d}.pt',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default='runs/checkpoint-diagnosis-seed17')
