"""Draw release-coordinate camera context for reviewed development images only.
...
def prepare(index_path, manifest_path, output):
    ...
    entries = []
    yaws, colors = (0, 90, 180, -90), ('#ffd859', '#ff7878', '#74dfa9', '#70b7ff')
    for sid, entry in sorted(index.items(), key=lambda pair: pair[1]['review_view_number']):
        ...
        if (row['yaw'], row['pitch'], row['fov'], row['size']) != (0, 0, 90, 256):
            raise ResearchError('This context layout expects the reviewed yaw-zero 90-degree crops.')
        ...
        target = crop_panorama(panorama, 0, 0, 90, 256)
        ...
        origin, directions, exits = geometry(row, 0)
        ...
        for j, (yaw, color) in enumerate(zip(yaws, colors)):
            ...
            draw.text((x, y), f'Yaw {yaw}' + (' | reviewed target' if yaw == 0 else ''), font=font, fill=color)
        ...
        draw.text((12, 702), 'White dot: released camera position. Yellow wedge: yaw 0, 90-degree horizontal field of view.', font=font, fill='white')
        ...
        entries.append(dict(sample_id=sid, review_view_number=number, source_satellite_sha256=sha256(sat_path),
                            source_panorama_sha256=sha256(pano_path), camera_xy=origin.tolist(), rays=ray_records,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--index', default='data/review/manual-scored-first3/approved-index.json')
